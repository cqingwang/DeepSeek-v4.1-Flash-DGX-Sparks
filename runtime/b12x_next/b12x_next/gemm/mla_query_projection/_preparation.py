...
def compile_mxfp8(query_payload, device_ordinal):
    from b12x_next.gemm._shared import mxfp8_bmm as kernels
    query = ProjectionQuery(**query_payload)
...
def plan(query: ProjectionQuery, *, invocation=FrozenMapping(), override=None) -> Plan:
    ...
    def compile_jobs(config, device):
        del config
        factory = "b12x_next.gemm.mla_query_projection._preparation:compile_bf16" if query.weight_format == "bf16" else "b12x_next.gemm.mla_query_projection._preparation:compile_mxfp8"
