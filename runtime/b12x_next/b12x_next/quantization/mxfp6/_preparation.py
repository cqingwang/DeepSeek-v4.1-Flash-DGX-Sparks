...
from b12x_next._lib.fp6 import as_grouped_mxfp6_scale_view
...
from b12x_next.preparation.types import _owned_tensor_nbytes
...
class _Mxfp6DenseState:
    ...
    def run(self, x: torch.Tensor, weight: torch.Tensor, weight_scales: torch.Tensor,
        ...
        if self.query.max_tokens <= 16:
            ...
            from b12x_next._lib.fp6 import mx_gs_numerator
            scale = (
...
def plan(query: Mxfp6DenseQuery, *, invocation=FrozenMapping(), override=None) -> Plan:
    ...
    def jobs(config, device):
        return (CompileJob.create("b12x_next.quantization.mxfp6._preparation:compile_mxfp6_dense", TUNING.encode_query(query), TUNING.encode_config(config), device.ordinal, device.identity.sm_count),)
    ...
    def materialize(selection, device):
        ...
        dense = __import__("b12x_next._lib.dense_gemm", fromlist=["_DenseExecutionState"])
