...
def plan(query: PcieQuery, *, runtime, invocation=FrozenMapping(), override: PcieConfig | None = None) -> Plan:
    ...
    return Plan(contract=TUNING, query=query, invocation=FrozenMapping(), override=override, _device=_runtime_device(runtime), _compile_jobs=lambda _config, device: (CompileJob.create("b12x_next.comm.pcie._owner_preparation:compile_owner_surface", TUNING.encode_query(query), device.ordinal),), _memory_requirements=memory, _materialize=materialize)
