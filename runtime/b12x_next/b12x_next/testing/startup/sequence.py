...
from b12x_next.preparation import PreparedCall, PreparationRequest
...
def _delta_requests(metadata, device, rows):
    from b12x_next.sequence import gdn_decode as decode
    ...
    prefill = importlib.import_module(f"b12x_next.sequence.{recipe}_prefill")
    requests = []
    for m in rows:
...
def _feedback_requests(metadata, device, rows):
    from b12x_next.sequence import mtp_feedback as op
...
def test_expected(call: PreparedCall):
    ...
    if context.recipe == "mtp":
        from b12x_next.sequence.mtp_feedback.reference import feedback
    ...
    if context.mode == "decode":
        from b12x_next.sequence.gdn_decode import reference
    ...
    op = importlib.import_module(f"b12x_next.sequence.{context.recipe}_prefill")
    fn = (
