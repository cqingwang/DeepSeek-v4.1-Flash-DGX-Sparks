"""b12x_next — consumer-Blackwell (SM120/SM121) kernels.
...
- ops live at ``b12x_next.<group>.<op>`` and declare themselves via ``META``;
...
def find_op(qualname: str) -> OpMeta:
    ...
    if qualname not in _OPS:
        raise KeyError(
            f"unknown experimental b12x_next op {qualname!r}; known ops: {sorted(_OPS)}"
        )
