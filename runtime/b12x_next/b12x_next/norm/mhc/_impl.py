...
from b12x_next._lib.scratch_layout import (
    SCRATCH_ALIGN_BYTES,
    align_up,
    dtype_nbytes,
    materialize_scratch_view,
)
from b12x_next.preparation.types import Plan, plan_from_handle, require_prepared
...
@torch.library.custom_op("b12x_next::mhc_pre_planned_functional", mutates_args=())
...
@torch.library.custom_op(
    "b12x_next::mhc_pre_lagged_planned_functional", mutates_args=("pre_out",)
)
...
@torch.library.custom_op("b12x_next::mhc_post_pre_planned_functional", mutates_args=())
...
@torch.library.custom_op(
    "b12x_next::mhc_post_pre_lagged_planned_functional", mutates_args=("pre_out",)
)
...
@torch.library.custom_op("b12x_next::mhc_post_planned_functional", mutates_args=())
...
@torch.library.custom_op("b12x_next::mhc_collapse", mutates_args=("out",))
...
def b12x_mhc_pre(
    ...
    if functional and pre_mix is not None:
        return torch.ops.b12x_next.mhc_pre_lagged_planned_functional(
    ...
    if functional:
        return torch.ops.b12x_next.mhc_pre_planned_functional(
...
def b12x_mhc_post_pre(
    ...
    if functional and (pre_mix is None) != (pre_out is None):
        raise ValueError("pre_mix and caller-owned pre_out must be supplied together")
    if functional and pre_mix is not None:
        return torch.ops.b12x_next.mhc_post_pre_lagged_planned_functional(
    ...
    if functional:
        return torch.ops.b12x_next.mhc_post_pre_planned_functional(
...
def b12x_mhc_post(x, residual, prev_post, prev_comb, *, plan: Plan, out=None):
    if out is None:
        return torch.ops.b12x_next.mhc_post_planned_functional(x, residual, prev_post, prev_comb, plan.handle)
    if torch.compiler.is_compiling():
...
def run_collapse(state, pre_mix, *, out, plan: Plan):
    torch.ops.b12x_next.mhc_collapse(state, pre_mix, out, plan.handle)
    return out
