...
def _threads_from_env() -> int:
    ...
    raw = os.getenv("B12X_NEXT_PCIE_ISLAND_RS_THREADS", "512")
    threads = int(raw)
    if not 32 <= threads <= 1024 or threads % 32:
        raise ValueError(
            "B12X_NEXT_PCIE_ISLAND_RS_THREADS must be a multiple of 32 in [32, 1024]"
        )
    return threads


def _wait_nanosleep_cycles_from_env() -> int:
    cycles = int(os.getenv("B12X_NEXT_PCIE_ISLAND_RS_NANOSLEEP_CYCLES", "24"))
    if not 0 <= cycles <= 1024:
        raise ValueError("B12X_NEXT_PCIE_ISLAND_RS_NANOSLEEP_CYCLES must be in [0, 1024]")
    return cycles
