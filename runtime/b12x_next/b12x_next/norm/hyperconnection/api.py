...
def run_grouped_rmsnorm(state, weight, *, eps, binding: Binding, zero_centered=True):
    ...
    torch.ops.b12x_next.hyperconnection_grouped_rmsnorm(
...
def run_scaled_silu(projected_down, *, binding: Binding):
    ...
    torch.ops.b12x_next.hyperconnection_scaled_silu(
...
def run_gate_mean(normalized, gate_logits, *, binding: Binding):
    ...
    torch.ops.b12x_next.hyperconnection_gate_mean(
...
def run_combine(state, block_output, injection_logits, *, plan: Plan):
    return torch.ops.b12x_next.hyperconnection_combine(state, block_output, injection_logits, plan.handle)


def run_combine_norm(state, block_output, injection_logits, next_norm_weight, *, eps, plan: Plan):
    return torch.ops.b12x_next.hyperconnection_combine_norm(
...
def run_engram_mix(state, projected_kv, norm_weights, *, eps, plan: Plan, out, token_mask=None):
    torch.ops.b12x_next.hyperconnection_engram_mix(
...
def run_swiglu(gate_up, *, limit, out, plan: Plan, round_silu=False):
    torch.ops.b12x_next.hyperconnection_swiglu(gate_up, out, float(limit), bool(round_silu), plan.handle)
    return out


def run_add(left, right, *, out, plan: Plan):
    torch.ops.b12x_next.hyperconnection_add(left, right, out, plan.handle)
    return out


def run_sigmoid(source, *, out, plan: Plan):
    torch.ops.b12x_next.hyperconnection_sigmoid(source, out, plan.handle)
    return out
