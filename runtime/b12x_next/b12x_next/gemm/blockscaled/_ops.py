...
@torch.library.custom_op("b12x_next::blockscaled_bf16", mutates_args=())
...
@torch.library.custom_op("b12x_next::blockscaled_bf16_workspace", mutates_args=("workspace",))
...
@torch.library.custom_op("b12x_next::blockscaled_bf16_out", mutates_args=("out",))
...
@torch.library.custom_op("b12x_next::blockscaled_bf16_out_workspace", mutates_args=("out", "workspace"))
...
def linear(source, values, scales, global_scale, *, plan: Plan,
    ...
    if out is None:
        if workspace is None:
            return torch.ops.b12x_next.blockscaled_bf16(*args, global_scale_kind, stream_int, handle, required_mode)
        return torch.ops.b12x_next.blockscaled_bf16_workspace(*args, workspace, global_scale_kind, stream_int, handle, required_mode)
    if workspace is None:
        torch.ops.b12x_next.blockscaled_bf16_out(*args, out, global_scale_kind, stream_int, handle, required_mode)
    else:
        torch.ops.b12x_next.blockscaled_bf16_out_workspace(*args, out, workspace, global_scale_kind, stream_int, handle, required_mode)
    return out
