...
def _qsa_requests(meta, device, rows):
    from b12x_next.attention import qsa
...
def _glm_requests(meta, device, rows):
    from b12x_next.attention import dsa_indexer, sparse_mla
...
def test_expected(call: PreparedCall):
    ...
    data = context.data
    if context.kind == "qsa":
        from b12x_next.attention.qsa.reference import (
    ...
    if context.kind == "indexer":
        from b12x_next.attention.dsa_indexer.reference import paged_decode_logits_reference
    ...
    if context.kind == "mla":
        from b12x_next.attention._shared.mla.reference import sparse_mla_reference
