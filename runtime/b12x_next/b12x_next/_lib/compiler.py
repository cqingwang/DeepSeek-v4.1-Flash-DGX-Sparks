...
# part of every disk-cache key.  Must resolve to b12x_next.
...
_VLLM_ENGINE_STARTED_ENV = "B12X_NEXT_ENGINE_STARTED"
_POST_ENGINE_START_LOG_ENV = "B12X_NEXT_LOG_CUTE_COMPILES_AFTER_ENGINE_START"
_PRINT_COMPILE_PROGRESS_ENV = "B12X_NEXT_PRINT_COMPILE_PROGRESS"
...
def _spec_memo_enabled() -> bool:
    raw = os.environ.get("B12X_NEXT_COMPILE_SPEC_MEMO", "1")
...
def _cute_compile_memory_cache_enabled() -> bool:
    raw = os.environ.get("B12X_NEXT_COMPILE_MEMORY_CACHE", "1")
    return raw.lower() not in {"0", "false", "no", ""}


def _cute_compile_memory_cache_size() -> int:
    raw = os.environ.get("B12X_NEXT_COMPILE_MEMORY_CACHE_SIZE", "1024")
    try:
...
def _cute_compile_disk_cache_enabled() -> bool:
    raw = os.environ.get("B12X_NEXT_COMPILE_DISK_CACHE", "1")
    return raw.lower() not in {"0", "false", "no", ""}
...
def _cute_compile_cache_dir() -> Path:
    root = os.environ.get("B12X_NEXT_COMPILE_CACHE_DIR")
    if root:
        return Path(root)
    xdg_cache_home = os.environ.get("XDG_CACHE_HOME")
    if xdg_cache_home:
        return Path(xdg_cache_home) / "b12x_next" / "compile"
    return Path.home() / ".cache" / "b12x_next" / "compile"


def _cute_compile_log_enabled() -> bool:
    raw = os.environ.get("B12X_NEXT_LOG_CUTE_COMPILES", "")
...
def _cute_compile_stack_log_enabled() -> bool:
    raw = os.environ.get("B12X_NEXT_LOG_CUTE_COMPILE_STACK", "")
    if raw:
        return raw.lower() not in {"0", "false", "no", "off"}
    raw = os.environ.get("B12X_NEXT_LOG_CUTE_COMPILES", "")
...
def _cute_compile_stack_log_depth() -> int:
    raw = os.environ.get("B12X_NEXT_LOG_CUTE_COMPILE_STACK_DEPTH", "")
    if not raw:
        return 48
    try:
        return max(1, int(raw))
...
def _cute_compile_arg_log_enabled() -> bool:
    raw = os.environ.get("B12X_NEXT_LOG_CUTE_COMPILE_ARGS", "")
    return raw.lower() not in {"", "0", "false", "no", "off"}
...
def _format_cute_compile_stack() -> str:
    ...
    lines = ["[b12x_next cute.compile] python_stack (most recent call last):"]
...
def _log_cute_compile_event(
    ...
    *,
    ...
    if _is_explicit_spec_payload(cache_payload):
        ...
        print(
            f"[b12x_next cute.compile] {event} "
            f"{reason_text}"
...
def _call_cute_compile(
    ...
    *,
    ...
    print(
        f"[b12x_next cute.compile] compile-start number={compile_number} {details}",
        flush=True,
    )
    ...
    try:
        ...
        print(
            f"[b12x_next cute.compile] compile-failed number={compile_number} "
    ...
    with _COMPILE_PROGRESS_LOCK:
        ...
        total = _COMPILE_PROGRESS_TOTAL_SECONDS
    print(
        f"[b12x_next cute.compile] compile-done number={compile_number} "
        f"duration_s={elapsed:.3f} total_compile_s={total:.3f} {details}",
        flush=True,
    )
    return compiled
...
def _compile_environment_key() -> tuple[tuple[str, str], ...]:
    ...
    operational_env_vars = {
        ...
        "B12X_NEXT_TUNING_CACHE_VERSION",
        "B12X_NEXT_COMPILE_CACHE_DIR",
        "B12X_NEXT_COMPILE_DISK_CACHE",
        "B12X_NEXT_COMPILE_MEMORY_CACHE",
        "B12X_NEXT_COMPILE_MEMORY_CACHE_SIZE",
        "B12X_NEXT_COMPILE_SPEC_MEMO",
        "B12X_NEXT_LOG_CUTE_COMPILES",
        "B12X_NEXT_LOG_CUTE_COMPILES_AFTER_ENGINE_START",
        "B12X_NEXT_LOG_CUTE_COMPILE_ARGS",
        "B12X_NEXT_LOG_CUTE_COMPILE_STACK",
        "B12X_NEXT_LOG_CUTE_COMPILE_STACK_DEPTH",
        "B12X_NEXT_PRINT_COMPILE_PROGRESS",
        "B12X_NEXT_PREPARATION_TRACE_DIR",
        "B12X_NEXT_TIMING",
        "B12X_NEXT_TIMING_THRESHOLD_MS",
        "CUTE_DSL_CACHE_DIR",
    }
    for name in os.environ:
        if name.startswith(("B12X_NEXT_", "CUTE_", "CUTLASS_")):
...
def _function_fingerprint(func: Any) -> tuple[str, str, str]:
    ...
    if module.startswith("b12x_next"):
        return module, qualname, f"b12x_next:{_b12x_package_fingerprint()}"
    try:
...
def _build_compile_manifest(
    ...
    manifest: dict[str, Any] = {
        "schema": "b12x_next._lib.compile_manifest.v3",
        "cache_key": cache_key,
        "cache_format": cache_format,
