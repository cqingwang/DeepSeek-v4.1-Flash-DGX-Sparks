...
from b12x_next._lib.compile_plan import attach_programs, compile_only_launches
from b12x_next._lib.compile_pool import CompileJob
from b12x_next._lib.program_cache import program_cache
from b12x_next._lib.scratch import scratch_buffer_spec
from b12x_next.preparation import FrozenMapping, MemoryRequirements, Plan
from ._tuning import MhcConfig, MhcQuery, TUNING

_CONTROL_NAMES = (
    "B12X_NEXT_AUTOTUNE_EXHAUSTIVE",
    "B12X_NEXT_MHC_PREFILL_TF32_MMA", "B12X_NEXT_MHC_PREFILL_BF16_MMA",
    "B12X_NEXT_MHC_PREFILL_BF16_MIN_TOKENS", "B12X_NEXT_MHC_PREFILL_TF32_MIN_TOKENS",
    "B12X_NEXT_MHC_PREFILL_MIN_TOKENS", "B12X_NEXT_MHC_PREFILL_BLOCK_M",
    "B12X_NEXT_MHC_PREFILL_BLOCK_M_SIZE", "B12X_NEXT_MHC_PREFILL_TILE_N",
    "B12X_NEXT_MHC_PREFILL_COMPACT", "B12X_NEXT_MHC_PREFILL_BF16_TMA",
    "B12X_NEXT_MHC_DECODE_SPLITS", "B12X_NEXT_MHC_DECODE_TILE_N",
    "B12X_NEXT_MHC_DECODE_BF16X2", "B12X_NEXT_MHC_PARTIALS_PER_CTA",
    "B12X_NEXT_MHC_DECODE_FINALIZE_THREADS", "B12X_NEXT_MHC_DECODE_FINALIZE_CTAS",
)
_CODEGEN_NAMES = (
...
def _lower_native(query, config, device):
    ...
    if query.operation == "pre" and query.lagged_mix and config.backend == "tf32_tma":
        ...
        threshold = int(controls.get("B12X_NEXT_MHC_PREFILL_MIN_TOKENS", "96"))
        bf16_threshold = int(controls.get("B12X_NEXT_MHC_PREFILL_BF16_MIN_TOKENS", "384"))
        if config.backend == "tf32_tma":
            route = "tf32"
        elif (query.has_norm_weight and query.has_fn_bf16 and tokens >= bf16_threshold
                and controls.get("B12X_NEXT_MHC_PREFILL_BF16_MMA", "1") != "0"):
            route = "bf16_tma" if controls.get("B12X_NEXT_MHC_PREFILL_BF16_TMA", "1") != "0" else "bf16_mma"
        elif (query.has_norm_weight and tokens >= threshold
                and controls.get("B12X_NEXT_MHC_PREFILL_BLOCK_M", "1") != "0"):
            route = "block_m"
        elif (query.has_norm_weight and tokens >= threshold
                and controls.get("B12X_NEXT_MHC_PREFILL_COMPACT", "1") != "0"):
            route = "compact"
        else:
            route = "decode"
    ...
    bf16x2 = False
    if route == "decode":
        raw = controls.get("B12X_NEXT_MHC_DECODE_SPLITS")
        if raw is not None and raw != "":
            source_splits = int(raw)
            tile_n = int(controls.get("B12X_NEXT_MHC_DECODE_TILE_N", "3")) if source_splits else 0
        ...
        raw = controls.get("B12X_NEXT_MHC_DECODE_BF16X2")
    ...
    if route in ("pre", "decode", "post_pre"):
        raw = controls.get("B12X_NEXT_MHC_PARTIALS_PER_CTA")
    ...
    if route in ("pre", "decode"):
        raw = controls.get("B12X_NEXT_MHC_DECODE_FINALIZE_THREADS")
        if raw is not None and raw != "":
            threads = int(raw)
        ...
        if threads:
            raw = controls.get("B12X_NEXT_MHC_DECODE_FINALIZE_CTAS")
    ...
    return _NativeLaunch(
        ...
        int(controls.get("B12X_NEXT_MHC_PREFILL_BLOCK_M_SIZE", "2")),
        int(controls.get("B12X_NEXT_MHC_PREFILL_TILE_N", "12" if hidden == 7168 else "24")),
    )
...
def plan_mhc(caps, *, invocation=FrozenMapping(), override=None):
    ...
    return Plan(
        ...
        _compile_jobs=lambda config, device: (CompileJob.create(
            "b12x_next.norm.mhc._preparation:compile_mhc", TUNING.encode_query(query),
