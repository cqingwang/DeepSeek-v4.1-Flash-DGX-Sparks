...
def _build() -> Path:
    ...
    if not compiler or not shutil.which(compiler[0]):
        raise RuntimeError("b12x_next.loader needs a C99 compiler (CC or cc)")
    ...
    for path in headers:
        if not path.is_file():
            raise RuntimeError(f"b12x_next.loader build dependency is missing: {path}")
    flags = [
        "-O2",
        "-std=c99",
        "-shared",
    ...
    cache = root / "b12x_next" / "loader" / digest
    ...
    if not target.exists():
        with tempfile.TemporaryDirectory(dir=cache) as temporary:
            ...
            if result.returncode:
                raise RuntimeError("b12x_next.loader native build failed:\n" + result.stderr)
...
def load():
    with _LOCK:
        ...
        if module.ABI_VERSION != 1:
            raise RuntimeError("b12x_next.loader native ABI mismatch")
        return module
