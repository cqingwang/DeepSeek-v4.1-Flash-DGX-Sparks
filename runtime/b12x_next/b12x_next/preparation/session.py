...
from b12x_next._lib.compile_plan import (
    compiled_program_available, observe_programs, retain_compiled_programs,
)
from b12x_next._lib.compile_pool import CompilePool, describe_compilation, compile_in_process
from b12x_next._lib.runtime_control import KernelResolutionFrozenError, kernel_resolution_guard
...
logger = logging.getLogger("b12x_next.preparation")
...
class PreparationSession:
    def __init__(
        ...
        if compile_workers is None:
            compile_workers = int(os.environ.get("B12X_NEXT_COMPILE_WORKERS", "8"))
        for name, value in (
        ...
        if os.environ.get("B12X_NEXT_AUTOTUNE", "1") == "0":
            self._stop.set()
    ...
    def configure_compile_workers(self, workers: int | None = None) -> None:
        ...
        if workers is None:
            workers = int(os.environ.get("B12X_NEXT_COMPILE_WORKERS", "8"))
    ...
    def _selection_cache(self):
        if self._cache is None:
            ...
            root = self.cache_dir
            if root is None:
                from b12x_next._lib.compiler import _cute_compile_cache_dir
...
class PreparationJob:
    def __init__(self, session, requests, *, autotune):
        ...
        if session.device.ordinal is not None:
            from b12x_next._lib.program_cache import stack_limit_bytes
    ...
    def _direct_compilation(self):
        ...
        if self.session._pool is not None:
            pool, self.session._pool = self.session._pool, None
            pool.close(terminate=True)
        from b12x_next._lib.compile_plan import evict_planning_artifacts
    ...
    def _run(self):
        ...
        if summary is not None:
            self._compilations = summary.cute_compilations + summary.triton_compilations
        from b12x_next._lib.program_cache import evict_unretained
        keep = frozenset(
        ...
        if self._benchmarked:
            ...
            from b12x_next._lib.program_cache import reclaim_device_memory
            self.session._synchronize()
            with self.session._gpu_scope():
...
def prepare_default(request):
    ...
    """
    from b12x_next._lib.runtime_control import kernel_resolution_frozen
