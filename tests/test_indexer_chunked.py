"""CPU equivalence test: chunked dense indexer (sglang#39187 backport) vs the stock
function lifted verbatim from this image's deepseek_v4_backend.py, both driven by the
...
BACKEND_SRC = sys.argv[1] if len(sys.argv) > 1 else "backend-ours.py"
TOPK = 4
HEADS = 2
...
sys.path.insert(0, ".")
import indexer_chunked  # noqa: E402
...
def build(seq_lens, q_lens, ratio, source, consume_masks=None, tail_lens=None, topk=TOPK):
    ...
    x = torch.zeros(T, 8)
    fb = SimpleNamespace(seq_lens_cpu=torch.tensor(seq_lens), req_pool_indices=torch.arange(len(seq_lens)))
    tail = None
    ...
    self = SimpleNamespace(
        token_to_kv_pool=Pool(), forward_metadata=SimpleNamespace(core_metadata=core),
...
def run(fn, budget, **kw):
    ...
    return self.forward_metadata.core_metadata._page.clone(), self.forward_metadata.core_metadata._raw.clone(), self.candidate_masks
