...
from b12x_next._lib.compile_plan import attach_programs
...
from b12x_next._lib.quant.lut_e4m3 import lut_e4m3_value_table
from b12x_next.moe._shared.kernels.trellis_ring import (
    trellis256_lane_geom_bits as _trellis_ring_lane_geom_bits,
)
from b12x_next.moe._shared.trellis_codebooks import (
    MCG,
    LUT_E4M3,
    LUT_FP16,
    validate_codebook_bits,
)
from b12x_next._lib.quant.lut_fp16 import (
    lut_fp16_segment_table,
)
from b12x_next._lib.utils import current_cuda_stream, make_ptr
from b12x_next.moe._shared.kernels.w4a16.route_pack import (
...
def _w4a16_small_m_splitk_enabled() -> bool:
    ...
    return os.environ.get("B12X_NEXT_W4A16_SMALL_M_SPLITK", "0") == "1"
...
def _lut_e4m3_smem_enabled() -> bool:
    ...
    return os.environ.get("B12X_NEXT_LUT_E4M3_SMEM", "1") == "1"
    ...
    def _run_persistent_gemm(
        ...
        if cutlass.const_expr(not self.schedule_whole_tiles):
            if cutlass.const_expr(self.uses_m_block_8 and not self.small_m_splitk):
                ...
                # via ``B12X_NEXT_W4A16_SMALL_M_SPLITK`` (see
...
def _small_m_direct_supported(
    *,
    ...
    if os.environ.get("B12X_NEXT_W4A16_SMALL_M_DIRECT", "1") == "0":
        return False
    activation = normalize_moe_activation(activation)
    swiglu_limit, swiglu_alpha, swiglu_beta = _normalize_activation_swiglu_params(
        activation,
...
def _small_m_direct_host_barrier_reset_enabled() -> bool:
    ...
    """
    return os.environ.get("B12X_NEXT_W4A16_SMALL_M_HOST_BARRIER_RESET", "1") != "0"
...
@torch.library.custom_op(
    "b12x_next::w4a16_small_m_direct_launch",
...
@torch.library.custom_op(
    "b12x_next::w4a16_fc2_direct_launch",
    mutates_args="unknown",
)
...
@torch.library.custom_op(
    "b12x_next::w4a16_fused_moe_launch",
    mutates_args="unknown",
)
...
@torch.library.custom_op(
    "b12x_next::w4a16_fused_moe_calibrated_launch",
    mutates_args="unknown",
)
...
@torch.library.custom_op(
    "b12x_next::w4a16_topk_sum_launch",
    mutates_args=("output",),
)
    ...
    *,
    ...
    if (
        ...
        if direct_launch is None:
            torch.ops.b12x_next.w4a16_small_m_direct_launch(*launch_args)
        else:
    ...
    if use_x4t_scale_predecode:
        ...
        from b12x_next._lib.quant.x4t_scales import (
    ...
    if collect_activation_amax:
        ...
        torch.ops.b12x_next.w4a16_fused_moe_calibrated_launch(
            *launch_common,
            activation_amax,
            int(layer_idx_int),
            *launch_tail,
        ...
        torch.ops.b12x_next.w4a16_fused_moe_launch(
            *launch_common,
            *launch_tail,
            bool(use_direct_topk_routes),
            bool(use_tc_decode),
