...
from b12x_next.moe._shared.kernels.w4a16.prepare import PreparedTrellis256DenseWeight
...
def _compile_trellis(query_payload, config_payload, ordinal, sm_count):
    ...
    from b12x_next.moe._shared.kernels.w4a16.kernel import compile_w4a16_gemm
...
def _c_tmp_elements(query: TrellisQuery, config: TrellisConfig, sm_count: int) -> int:
    from b12x_next.moe._shared.kernels.w4a16.kernel import packed_gemm_scratch_elements
...
def _lut_memory(query: TrellisQuery, device) -> PersistentMemory | None:
    ...
    if query.codebook == "lut_e4m3":
        from b12x_next._lib.quant.lut_e4m3 import lut_e4m3_value_table_resident
...
class _TrellisExecutionState:
    ...
    def run(self, x, *, output=None, gemm_output=None, input_f16=None,
            ...
            output_f16=None):
        from b12x_next.moe._shared.kernels.w4a16.kernel import run_trellis256_dense
...
def plan(query: TrellisQuery, *, weight: PreparedTrellis256DenseWeight,
    ...
    def compile_jobs(config, device):
        return (CompileJob.create(
            "b12x_next.gemm.trellis_linear._preparation:_compile_trellis",
    ...
    def materialize(selection, device):
        from b12x_next.moe._shared.kernels.w4a16.kernel import _W4A16GemmLaunch, _resolve_exl3_hadamard_128, _trellis256_execution_lut
