...
from b12x_next.preparation import FrozenMapping, PreparedCall, PreparationRequest
...
def _embedding_requests(metadata, device, rows):
    from b12x_next.sequence import ple_embedding, ple_hash
...
def _convolution_requests(metadata, device, rows):
    from b12x_next.sequence import ple as op
...
def _expected_ids(binding):
    from b12x_next.sequence.ple_hash.reference import ple_hash_packed_reference
...
def test_expected(call: PreparedCall):
    ...
    if context.role == "convolution":
        from b12x_next.sequence.ple.reference import ple_projected_packed_reference
    ...
    if b.disk_table is None:
        from b12x_next.sequence.ple_embedding.reference import lookup
    ...
    from b12x_next.sequence.ple_embedding.reference import _dequantize_selected_nvfp4
