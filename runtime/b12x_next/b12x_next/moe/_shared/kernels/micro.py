...
_BLOCK_SIZE = 16
# MX-FP6 block scales use 32 elements per vector (see b12x_next._lib.fp6).
MXFP6_BLOCK_SIZE = 32
_FP8_E4M3_MAX = 448.0
...
# primitive (b12x_next._lib.fp6) instead of the FP4 software dot.
...
def _fp6_micro_enabled() -> bool:
    ...
    """
    return os.environ.get("B12X_NEXT_ENABLE_FP6_MICRO", "0").strip().lower() not in (
        "",
        "0",
        "false",
        "no",
...
class MoEMicroKernelBackend:
    ...
    def is_supported_mxfp6(
        ...
        Opt-in via ``B12X_NEXT_ENABLE_FP6_MICRO`` (default off) so the
