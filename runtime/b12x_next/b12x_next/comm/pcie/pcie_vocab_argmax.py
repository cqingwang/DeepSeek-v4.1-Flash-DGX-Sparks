...
def _wait_nanosleep_cycles_from_env() -> int:
    raw = os.getenv("B12X_NEXT_PCIE_VOCAB_ARGMAX_NANOSLEEP_CYCLES", "24")
    try:
        cycles = int(raw)
    except ValueError as exc:
        raise ValueError(
            "B12X_NEXT_PCIE_VOCAB_ARGMAX_NANOSLEEP_CYCLES must be an integer"
        ) from exc
    if not 0 <= cycles <= 1024:
        raise ValueError("B12X_NEXT_PCIE_VOCAB_ARGMAX_NANOSLEEP_CYCLES must be in [0, 1024]")
    return cycles
