...
def _caps(query: DenseMlaQuery, config: DenseMlaConfig, device: torch.device) -> Caps:
    return Caps(
        ...
        budget=__import__("b12x_next.attention.dense_mla.planner", fromlist=["Budget"]).Budget(max_splits=config.max_splits),
    )
...
def plan(
    ...
    return Plan(
        ...
        _compile_jobs=lambda config, device: (CompileJob.create(
            "b12x_next.attention.dense_mla._preparation:compile_dense_mla",
