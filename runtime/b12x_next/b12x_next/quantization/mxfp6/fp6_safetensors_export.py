"""Emit a full HF safetensors checkpoint quantized to b12x_next MX-FP6 (W6A6).

Unlike the per-layer ``.pt`` converters in
:mod:`b12x_next.quantization.mxfp6.model_fp6` (useful for kernel validation),
...
Quantized linears follow the :mod:`b12x_next.quantization.mxfp6.fp6_checkpoint`
...
* MoE: routed experts are written as **per-expert** keys
  ...
  b12x_next FP4 loader, regardless of whether the source stored them packed.
...
def dequantize_fp6_checkpoint_to_bf16(
    ...
    *,
    ...
    The output runs on stock vLLM with no b12x_next involvement, so a KLD
    ...
    if qcfg.get("quant_algo") != QUANT_ALGO:
        raise ValueError(
            f"{model_path} is not a b12x_next FP6 checkpoint "
