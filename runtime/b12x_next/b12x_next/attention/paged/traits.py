...
from b12x_next._lib.smem import make_tma_aligned_payload_storage
    ...
    *,
    ...
    if (
        ...
        if (
            ...
            and plan.page_size == 128
            and plan.head_dim_qk == 128
            and paged_control("B12X_NEXT_PAGED_LAGUNA_DECODE_N128", "0") == "1"
        ):
            exact_num_mma_kv = 2
    ...
    force_wide_bf16_extend = (
        paged_control("B12X_NEXT_PAGED_EXTEND_BF16_N32", "0") == "1"
    )
    force_narrow_bf16_extend = (
        paged_control("B12X_NEXT_PAGED_EXTEND_BF16_N16", "0") == "1"
    )
