...
def _apply_decode_graph_chunk_pages_debug_policy(chunk_pages: int) -> int:
    forced = _decode_graph_chunk_pages_env(
        "B12X_NEXT_PAGED_DECODE_GRAPH_CHUNK_PAGES"
    )
    if forced is not None:
        return forced
    minimum = _decode_graph_chunk_pages_env(
        "B12X_NEXT_PAGED_DECODE_GRAPH_MIN_CHUNK_PAGES"
    )
    ...
    *,
    ...
    if msa_union_tile is None:
        msa_union_tile = (
            msa_block_sparse
            and mode == "extend"
            and paged_control("B12X_NEXT_PAGED_MSA_UNION_PREFILL", "1")
            != "0"
        )
    else:
    ...
    if (
        paged_control("B12X_NEXT_DEBUG_PAGED_POLICY") == "1"
        and enable_cuda_graph
        and graph_chunk_policy
        and mode == "decode"
    ):
