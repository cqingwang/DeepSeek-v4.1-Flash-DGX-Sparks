...
from b12x_next._lib.scratch import ScratchBufferSpec
from b12x_next._lib.utils import cuda_stream_to_int
...
from b12x_next.preparation.types import plan_from_handle, require_prepared
...
def _lower_query(query, device, constraints):
    ...
    from b12x_next._lib.dense_gemm import _lower_dense_gemm
...
def plan(query: DenseGemmQuery, *, invocation=FrozenMapping(), override=None):
    ...
    def materialize(selection, device):
        from b12x_next._lib.dense_gemm import _materialize_dense
    ...
    return Plan(
        ...
        _compile_jobs=lambda config, device: (CompileJob.create(
            "b12x_next._lib.dense_gemm:_compile_dense_lowering", lower(config, device).to_dict(), device.ordinal,
        ),),
...
def _result_metadata(a, b, output_dtype):
    from b12x_next._lib.dense_gemm import _empty_dense_gemm_output
    return _empty_dense_gemm_output(a.shape[0], b.shape[0], a.shape[2], dtype=output_dtype, device=a.device)


@torch.library.custom_op("b12x_next::dense_prepared_functional", mutates_args=())
...
@torch.library.custom_op("b12x_next::dense_prepared_functional_workspace", mutates_args=("workspace",))
...
@torch.library.custom_op("b12x_next::dense_prepared_out", mutates_args=("out",))
...
@torch.library.custom_op("b12x_next::dense_prepared_out_workspace", mutates_args=("out", "workspace"))
...
def mm(lhs, rhs, out=None, *, plan: Plan, alpha=None,
    ...
    if out is None:
        if workspace is None:
            return torch.ops.b12x_next.dense_prepared_functional(
                a, sfa, b, sfb, alpha, out_dtype, stream_int, handle,
            )
        return torch.ops.b12x_next.dense_prepared_functional_workspace(
            a, sfa, b, sfb, alpha, workspace, out_dtype, stream_int, handle,
        )
    if workspace is None:
        torch.ops.b12x_next.dense_prepared_out(
            a, sfa, b, sfb, alpha, out, out_dtype, stream_int, handle,
        )
    else:
        torch.ops.b12x_next.dense_prepared_out_workspace(
