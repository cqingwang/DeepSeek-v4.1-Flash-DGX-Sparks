...
from b12x_next.preparation._progress import (
...
class CheckpointDisplay:
    ...
    def _publish(self, **changes):
        ...
        if stage != self._last_stage or routed or now - self._last_log >= 10:
            ...
            self._stream.write(f"b12x_next {label.lower()}: {detail}\n")
            self._stream.flush()
    ...
    def _render(self):
        ...
        return Panel(grid, box=box.SQUARE, border_style=_TRACK, padding=(0, 2),
                     title=Text(" b12x_next / weight loading ", style=f"bold {_INK}"), title_align="left")
