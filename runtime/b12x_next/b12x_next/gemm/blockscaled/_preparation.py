...
from b12x_next._lib.quant.block_codec import BLOCK_CODECS, IQ2_CODECS, block_codec
...
from b12x_next._lib.utils import current_cuda_stream, cuda_stream_to_int, make_ptr
from b12x_next.preparation import FrozenMapping, MemoryRequirements, PersistentMemory, Plan
from b12x_next.preparation.types import _CompositePlan
from ._tuning import (
...
def _lower_dense_with_hint(query, config, device, expected_m):
    from b12x_next.gemm._preparation import _default_lowering
    from b12x_next.gemm._tuning import DenseGemmQuery
...
def compile_packed(query_payload, config_payload, dense_payload, short_dense_payload,
    ...
    import cutlass
    from b12x_next._lib import dense_gemm as dense
    ...
    with torch.cuda.device(ordinal):
        ...
        if functional_mxfp8_quantization(query, config):
            from b12x_next._lib.quant import mxfp8_rows
...
def _mxfp8_bases_nbytes(query):
    from b12x_next.gemm._shared.wo_mxfp8 import (
...
class _PackedExecutionState:
    ...
    def _mxfp8_rows(self, m):
        from b12x_next.gemm._shared.wo_mxfp8 import MXFP8_SCALE_ROW_TILE, mxfp8_rows_from_bases
    ...
    def run(self, source, values, scales, global_scale, *, activation_scale=None,
        ...
        with torch.cuda.device(self.device), _stream_context(stream, self.device):
            ...
            from b12x_next._lib.intrinsics import as_grouped_scale_view, as_grouped_scale_view_mx
...
def _plan_bf16(query: BlockscaledQuery, *, invocation=FrozenMapping(), override=None):
    ...
    def compile_jobs(config, device):
        ...
        return (CompileJob.create(
            "b12x_next.gemm.blockscaled._preparation:compile_packed",
    ...
    def memory(config, device):
        from b12x_next._lib import dense_gemm as dense
        from b12x_next.preparation import current_plan, current_prepared_state
        ...
        if query.recipe in IQ2_CODECS:
            from b12x_next._lib.quant.iq2_xs import _DEVICE_TABLES
    ...
    def materialize(selection, device):
        ...
        if query.workspace_form == "owned":
            from b12x_next.preparation import current_prepared_state
            existing = current_prepared_state()
            if needed:
            ...
            if functional_mxfp8_quantization(query, config):
                from b12x_next.gemm._shared.wo_mxfp8 import empty_mxfp8_rows_bases
        ...
        lut = None
        if query.recipe in IQ2_CODECS:
            from b12x_next._lib.quant.iq2_xs import iq2_xs_execution_lut
...
class _FixedExecutionState:
    ...
    def run_serialized(self, lhs_values, lhs_scale_storage, rhs_values, rhs_scale_storage, alpha,
        ...
        from b12x_next._lib.intrinsics import as_grouped_scale_view, as_grouped_scale_view_mx
        from b12x_next.gemm._tuning import DenseGemmQuery, operand_options
        q = self.query
    ...
    def run_mxfp8(self, source_values, weight_values, weight_scale_mma, *, source_scale=None, out_dtype, stream):
        ...
        with torch.cuda.device(self.device), _stream_context(stream, self.device):
            ...
            if q.input_dtype == "float16":
                ...
                from b12x_next.gemm._shared.wo_mxfp8 import empty_mxfp8_rows_bases, mxfp8_rows_from_bases
...
def _plan_fixed(query, *, invocation=FrozenMapping(), override=None):
    ...
    return Plan(
        ...
        _compile_jobs=lambda config, device: (CompileJob.create(
            "b12x_next.gemm.blockscaled._preparation:compile_fixed", FIXED_TUNING.encode_query(query),
...
def plan(query, *, invocation=FrozenMapping(), override=None):
    ...
    if isinstance(query, DenseGemmQuery):
        from b12x_next.gemm._preparation import plan as dense_plan
...
def query_from_call(source, weight, *, activation_mode="auto", activation_global_scale=None,
    ...
    if isinstance(source, tuple) and isinstance(weight, tuple) and source[0].ndim == 3:
        from b12x_next.gemm._tuning import query_from_call as dense_query
        recipe = dict(options)
