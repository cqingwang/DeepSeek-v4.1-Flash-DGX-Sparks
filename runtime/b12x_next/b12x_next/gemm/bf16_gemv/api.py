...
def is_disabled() -> bool:
    """True when ``B12X_NEXT_DISABLE_BF16_GEMV`` turns the routing off
    entirely (debug isolation switch)."""
    return os.environ.get("B12X_NEXT_DISABLE_BF16_GEMV", "").lower() in (
        "1",
        "true",
        "yes",
    )
