...
def _wait_nanosleep_cycles_from_env() -> int:
    raw = os.getenv("B12X_NEXT_PCIE_HIERARCHICAL_NANOSLEEP_CYCLES", "24")
    try:
        cycles = int(raw)
    except ValueError as exc:
        raise ValueError(
            "B12X_NEXT_PCIE_HIERARCHICAL_NANOSLEEP_CYCLES must be an integer"
        ) from exc
    if not 0 <= cycles <= 1024:
        raise ValueError(
            "B12X_NEXT_PCIE_HIERARCHICAL_NANOSLEEP_CYCLES must be in [0, 1024]"
        )
    return cycles


def _threads_from_env() -> int:
    raw = os.getenv("B12X_NEXT_PCIE_HIERARCHICAL_THREADS", "224")
    try:
        threads = int(raw)
    except ValueError as exc:
        raise ValueError(
            "B12X_NEXT_PCIE_HIERARCHICAL_THREADS must be an integer"
        ) from exc
    if not 32 <= threads <= 1024 or threads % 32 != 0:
        raise ValueError(
            "B12X_NEXT_PCIE_HIERARCHICAL_THREADS must be a multiple of 32 "
            "in [32, 1024]"
        )
    return threads
...
def _vectorized_bf16x2_from_env() -> bool:
    raw = os.getenv("B12X_NEXT_PCIE_HIERARCHICAL_BF16X2", "1")
    if raw not in ("0", "1"):
        raise ValueError("B12X_NEXT_PCIE_HIERARCHICAL_BF16X2 must be 0 or 1")
    return raw == "1"


def _vectorized_bf16x2_max_elements_from_env() -> int:
    raw = os.getenv(
        "B12X_NEXT_PCIE_HIERARCHICAL_BF16X2_MAX_ELEMENTS",
        "7168",
    )
    try:
        ...
        raise ValueError(
            "B12X_NEXT_PCIE_HIERARCHICAL_BF16X2_MAX_ELEMENTS must be an integer"
        ) from exc
    if not 0 <= max_elements <= 1 << 30:
        raise ValueError(
            "B12X_NEXT_PCIE_HIERARCHICAL_BF16X2_MAX_ELEMENTS must be in [0, 2**30]"
        )
    return max_elements
...
def _buffer_modes_from_env() -> tuple[bool, bool]:
    ...
    double_buffered = (
        os.getenv("B12X_NEXT_PCIE_HIERARCHICAL_DOUBLE_BUFFER", "0") == "1"
    )
    deferred_consumption = (
        os.getenv(
            "B12X_NEXT_PCIE_HIERARCHICAL_DEFERRED_CONSUMPTION",
            "0",
        )
        == "1"
    )
    if double_buffered and deferred_consumption:
        raise ValueError(
            "B12X_NEXT_PCIE_HIERARCHICAL_DOUBLE_BUFFER and "
            "B12X_NEXT_PCIE_HIERARCHICAL_DEFERRED_CONSUMPTION are "
            "mutually exclusive"
        )
