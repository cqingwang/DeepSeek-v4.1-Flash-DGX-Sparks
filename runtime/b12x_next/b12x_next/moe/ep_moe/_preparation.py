...
from b12x_next.moe._shared.kernels.w4a16.kernel import compile_w4a16_fused_moe
...
def plan(caps: EPMoEScratchCaps, *, expert_map: EPExpertMap, invocation: FrozenMapping = FrozenMapping(), override: EpMoeConfig | None = None):
    ...
    def child(tokens):
        ...
        def compile_jobs(config, device):
            return (CompileJob.create("b12x_next.moe.ep_moe._preparation:compile_ep_moe", TUNING.encode_query(query), invocation.to_dict(), weights, device.ordinal),)
        route_ids_workspace = (
