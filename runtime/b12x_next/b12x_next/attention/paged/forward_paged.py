...
from b12x_next.attention._shared.cute import copy as cute_copy
from b12x_next.attention._shared.cute import (
    pipeline as cute_pipeline,
)
from b12x_next.attention._shared.cute import ops as attention_ops
from b12x_next._lib.intrinsics import (
    get_ptr_as_int64,
    shared_ptr_to_u32,
)
from b12x_next._lib.smem import (
...
def _paged_kv_tma_plane_layout(stage_tile_rows: int, kv_tma_plane_head_dim: int):
    plane_swizzle = paged_control("B12X_NEXT_PAGED_KV_TMA_PLANE_SWIZZLE", "")
...
def _issue_paged_kv_tma_copy_planes_tma(
    ...
    page_id = (
        Int32(0)
        if const_expr(
            paged_control("B12X_NEXT_PAGED_KV_TMA_FORCE_PAGE0", "0") == "1"
        )
...
class PagedForwardKernel:
    ...
    def __init__(
        ...
        *,
        ...
        bf16_minimax_head128_decode = (
            ...
            and paged_control("B12X_NEXT_PAGED_BF16_MINIMAX_ROLE_SPECIALIZED", "0")
            == "1"
            and dtype_q == cutlass.BFloat16
        ...
        laguna_decode_n128 = (
            ...
            and self.gqa_group_size == 6
            and paged_control("B12X_NEXT_PAGED_LAGUNA_DECODE_N128", "0")
            == "1"
        )
        ...
        self.laguna_fp8_head_pair_decode = bool(
            ...
            and traits.head_dim_qk == 128
            and traits.head_dim_vo == 128
            and paged_control("B12X_NEXT_PAGED_LAGUNA_HEAD_PAIR_DECODE", "0")
            == "1"
        )
        self.laguna_fp8_head_pair_wide_tma = bool(
            self.laguna_fp8_head_pair_decode
            and paged_control("B12X_NEXT_PAGED_LAGUNA_HEAD_PAIR_WIDE_TMA", "0")
            == "1"
        )
        self.laguna_fp8_head_pair_wide_headlocal = bool(
            self.laguna_fp8_head_pair_wide_tma
            and paged_control("B12X_NEXT_PAGED_LAGUNA_HEAD_PAIR_WIDE_HEADLOCAL", "0")
            == "1"
        )
        ...
        tma_debug_dump = paged_control("B12X_NEXT_PAGED_KV_TMA_DEBUG_DUMP", "")
        paged_debug_dump = paged_control("B12X_NEXT_PAGED_KV_DEBUG_DUMP", "")
        ...
        self.use_m16n16_b8_fp8_decode_pv = (
            paged_control("B12X_NEXT_PAGED_DECODE_FP8_PV_M16N16_B8", "1") == "1"
            and decode_only
            and self.kv_is_fp8
            and dtype_q == cutlass.BFloat16
            and dtype_o == cutlass.BFloat16
    ...
    def _get_paged_kv_tma_head_pair_layout(self):
        if paged_control("B12X_NEXT_PAGED_LAGUNA_HEAD_PAIR_WIDE_NOSWIZZLE", "0") == "1":
            return cute.make_layout(
    ...
    def _get_paged_kv_tma_head_pair_stage_layout(self):
        ...
        if paged_control("B12X_NEXT_PAGED_LAGUNA_DEBUG_PRINT_HEAD_PAIR_LAYOUT", "0") == "1":
    ...
    def _issue_paged_kv_tma_copy_planes(
        ...
        page_id = (
            Int32(0)
            if const_expr(
                paged_control("B12X_NEXT_PAGED_KV_TMA_FORCE_PAGE0", "0")
                == "1"
            )
            else mPageTable[request_idx, page_idx]
        )
    ...
    def kernel(
        ...
        decode_qwen_single_row_fastpath = const_expr(
            self.decode_only
            and paged_control("B12X_NEXT_PAGED_GQA6_COMPACT_SYNC", "1") != "0"
            and not self.has_relative_attention_bias
            and (
                self.single_request_decode_graph
                or self.single_qtile_decode_graph
...
class PagedFp8DecodeRawForwardKernel:
    ...
    def kernel(
        ...
        while tile_base < cache_len:
            ...
            if const_expr(
                paged_control("B12X_NEXT_PAGED_KV_DEBUG_DUMP", "")
                == "SREGS"
            ):
                if (
                    kv_head_idx == Int32(0)
            ...
            for row_slot in cutlass.range_constexpr(2):
                ...
                for mma_d in cutlass.range_constexpr(self.num_mma_d_vo):
                    ...
                    if valid_row_store:
                        if const_expr(
                            self.split_kv
                            and paged_control("B12X_NEXT_DEBUG_BF16_EXTEND_DIRECT_STORE", "")
                            == "1"
                        ):
                            partial_row_idx = (
                                request_partial_start
