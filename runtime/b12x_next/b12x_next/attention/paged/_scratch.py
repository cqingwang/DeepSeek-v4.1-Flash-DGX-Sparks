...
by ``b12x_next.attention.paged._forward.paged_attention_forward``. Decode graph
...
if TYPE_CHECKING:
    from b12x_next.preparation.types import Plan


from b12x_next.attention.paged.planner import (
    PagedPlan,
    PagedPlanBudget,
    create_paged_plan,
    infer_paged_mode,
...
class B12XPagedAttentionScratchCaps:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            and os.environ.get("B12X_NEXT_PAGED_MSA") == "0"
        ):
            raise RuntimeError(
                "B12X_NEXT_PAGED_MSA=0 disables MSA block-sparse paged attention"
            )
        if self.msa_union_tile is None:
            msa_union_tile = (
                self.msa_block_sparse
                and self.mode == "extend"
                and os.environ.get("B12X_NEXT_PAGED_MSA_UNION_PREFILL", "1")
                != "0"
            )
        else:
...
class B12XPagedAttentionScratch:
    ...
    def update_decode_graph_replay_metadata_from_runtime_cache_seqlens(
        ...
        if not self._plan.split_kv:
            if int(self._plan.window_left) < 0:
                return self
            from b12x_next.attention.paged.graph_replay import (
    ...
    def _validate_q2k_indices_reference(
        ...
        if not self.msa_block_sparse:
            ...
            return
        if os.environ.get("B12X_NEXT_PAGED_MSA") == "0":
            raise RuntimeError(
                "B12X_NEXT_PAGED_MSA=0 disables MSA block-sparse paged attention"
            )
...
class B12XPagedAttentionBinding:
    ...
    def run(self) -> tuple[torch.Tensor, torch.Tensor]:
        from b12x_next.attention.paged._forward import (
