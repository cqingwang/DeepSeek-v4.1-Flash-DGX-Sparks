"""Framework enablement layer for serving b12x_next MX-FP6 (W6A6/W6A8) checkpoints.

This is the reference surface a vLLM / SGLang quantization backend calls to run an
FP6 checkpoint produced by :mod:`b12x_next.quantization.mxfp6.fp6_safetensors_export`.
It is framework-agnostic (no vLLM/SGLang imports) so it can be wired into any
fork; :mod:`b12x_next.integration.vllm.plugin` is the concrete vLLM adapter.

Enablement is twofold (mirrors how the FP4 b12x_next path is selected framework-side):

1. **Env gate** — ``B12X_NEXT_ENABLE_FP6=1`` must be set. If unset,
...
* prepared experts from :func:`b12x_next.moe.fused_moe.prepare_weights` with an
...
* weight from :func:`b12x_next.quantization.mxfp6.load_fp6_dense_checkpoint` as an
  :class:`~b12x_next.quantization.mxfp6.fp6_dense_weights.FP6DenseWeight`.

End-to-end flow
---------------

    pip install b12x_next
    python scripts/quantize_model_fp6.py --model <bf16 model> --out <fp6 model> --arch auto
    export B12X_NEXT_ENABLE_FP6=1
    vllm serve <fp6 model>      # plugin detects W6A6 and calls b12x_next
"""
...
ENABLE_ENV = "B12X_NEXT_ENABLE_FP6"
...
def is_b12x_fp6_enabled() -> bool:
    """True iff ``B12X_NEXT_ENABLE_FP6`` is set."""
...
def is_b12x_fp6_checkpoint(config: Any) -> bool:
    """True iff ``config`` declares a b12x_next FP6 (modelopt + W6A6) quantization."""
...
class B12XFP6MoEMethod:
    ...
    def get_b12x_preparation_units(self, layer, workload):
        ...
        from vllm.utils.b12x_next import B12xPreparationUnit
    ...
    def _scratch_for(self, tokens: int, plan: Any) -> tuple[torch.Tensor, ...]:
        ...
        if scratch is None:
            from b12x_next.preparation import require_prepared
...
class B12XFP6LinearMethod:
    ...
    def apply(
        ...
        from b12x_next.quantization.mxfp6 import dense_fp6_linear
...
def load_b12x_fp6_moe_methods(
    ...
    *,
    ...
    from b12x_next.moe import fused_moe
    from b12x_next.quantization.mxfp6 import load_fp6_moe_checkpoint
    ...
    for layer_idx, weights in layers.items():
        ...
        from b12x_next._lib.fp6 import unswizzle_mxfp6_scales
        ...
        from vllm.utils.b12x_next import register_b12x_unit_provider
...
def load_b12x_fp6_linear_methods(
    ...
    *,
    ...
    from b12x_next.quantization.mxfp6 import load_fp6_dense_checkpoint
