...
with the host C compiler into the b12x_next cache directory, so the package stays
...
def _traffic_class() -> int:
    ...
    for name in ("B12X_NEXT_ROCE_TRAFFIC_CLASS", "NCCL_IB_TC"):
        raw = os.getenv(name)
        if raw is not None:
            try:
                value = int(raw, 0)
...
def _cache_dir() -> Path:
    """Directory for the compiled proxy library: ``B12X_NEXT_ROCE_CACHE_DIR``, else ``<XDG cache>/b12x_next/roce``."""
    override = os.getenv("B12X_NEXT_ROCE_CACHE_DIR")
    if override:
        return Path(override)
    root = os.getenv("XDG_CACHE_HOME") or os.path.join(
        os.path.expanduser("~"), ".cache"
    )
    return Path(root) / "b12x_next" / "roce"
...
def _compiler() -> str:
    ...
    for candidate in (os.getenv("CC"), "gcc", "cc", "clang"):
        if candidate and shutil.which(candidate):
            return candidate
    raise RuntimeError(
        "b12x_next.comm.roce needs a C compiler (gcc/cc/clang) and libibverbs headers "
...
def _build() -> Path:
    ...
    if proc.returncode != 0:
        raise RuntimeError(
            "failed to build the b12x_next RoCE proxy: " + " ".join(cmd) + "\n" + proc.stderr
        )
...
def load() -> ctypes.CDLL:
    ...
    with _LOCK:
        ...
        if lib.roce_abi_version() != 4:
            raise RuntimeError("unexpected b12x_next RoCE proxy ABI version")
        _LIB = lib
        return lib
