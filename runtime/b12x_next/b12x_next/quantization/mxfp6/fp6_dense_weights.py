...
:mod:`b12x_next.quantization.mxfp6.fp6_moe_weights`. It targets ordinary
``nn.Linear`` weights (MLP gate/up/down, attention q/k/v/o projections) and the
block-scaled :func:`b12x_next._lib.dense_gemm.dense_gemm` kernel, rather than
fused-MoE experts.
...
from b12x_next._lib.fp6 import (
    SF_VEC_SIZE_FP6,
    as_grouped_mxfp6_scale_view,
    mx_gs_numerator,
)
from b12x_next._lib.utils import mxfp6_packed_k_bytes
from b12x_next._lib.dense_gemm import dense_gemm, _DENSE_FUSED_QUANT
...
_DENSE_PER_ROW_GS = os.getenv("B12X_NEXT_DENSE_PER_ROW_GS", "1").lower() not in (
    "0",
    "false",
)
_PERSISTENT_SCRATCH = os.getenv(
    "B12X_NEXT_DENSE_PERSISTENT_SCRATCH", "1"
).lower() not in ("0", "false")
# The fallback host chain and in-kernel per-row scaling are bit-identical.
_PER_ROW_IN_KERNEL = os.getenv(
    "B12X_NEXT_DENSE_PER_ROW_IN_KERNEL", "1"
).lower() not in ("0", "false")
...
def _small_m_quant_scratch(m_pad: int, k: int, device: torch.device) -> tuple:
    ...
    """
    from b12x_next.quantization.mxfp6 import allocate_bf16_to_fp6_tma_outputs
...
PACKED_GEMM_MIN_N = int(os.getenv("B12X_NEXT_PACKED_B_MIN_N", "12288"))
...
def _quantize_matrix_fp6(
    ...
    from b12x_next.quantization.mxfp6 import (
...
def _quantize_matrix_fp6_bytes_per_row(
    ...
    1. :func:`~b12x_next.quantization.mxfp6.fp6_row_gs.compile_fp6_row_gs`
    ...
    from b12x_next.quantization.mxfp6.fp6_row_gs import compile_fp6_row_gs
...
def _quantize_matrix_fp6_bytes_small_m(
    ...
    """
    from b12x_next.quantization.mxfp6.bf16_to_fp6_small_m import (
...
class FP6DenseWeight:
    ...
    def expanded_weight(self) -> torch.Tensor:
        ...
        if self._packed_expanded is None:
            # Lazy import: avoids any import cycle with ``b12x_next._lib.dense_gemm``.
            from b12x_next._lib.dense_gemm import _expand_packed_mxfp6_ab
