...
def _fused_tile_override():
    value = os.environ.get("B12X_NEXT_WO_B_FUSED_TILE", "").strip().lower()
    if value and value not in ("16x64", "16x128"):
        raise ValueError("B12X_NEXT_WO_B_FUSED_TILE must be 16x64 or 16x128")
    return value
...
def _fused_b_tile(query: WoProjectionQuery, config, device):
    ...
    if query.wo_b_tiled:
        from b12x_next._lib.dense_gemm import _select_default_dense_gemm_plan
...
def plan(caps: WOProjectionScratchCaps, *, invocation=FrozenMapping(), override=None):
    ...
    def fused_b(config, device):
        ...
        if key not in fused:
            from b12x_next._lib.dense_gemm import _lower_dense_gemm_fused_quant_a
            from b12x_next.gemm._preparation import _metadata_operands
    ...
    def ordinary_states(config, device):
        ...
        if key not in ordinary:
            from b12x_next.gemm._preparation import _default_lowering, _lower_query
            from b12x_next.gemm._tuning import DenseGemmQuery
    ...
    def memory(config, device):
        ...
        from b12x_next._lib.dense_gemm import _ALPHA_ONE_CACHE
    ...
    def materialize(selection, device):
        from b12x_next._lib.dense_gemm import _materialize_dense, _materialize_dense_fused_quant
    ...
    return Plan(
        ...
        _compile_jobs=lambda config, device: (
            CompileJob.create("b12x_next._lib.dense_gemm:_compile_dense_lowering",
                              ordinary_states(config, device)[0].to_dict(), device.ordinal),
            CompileJob.create("b12x_next._lib.dense_gemm:_compile_dense_lowering",
                              ordinary_states(config, device)[1].to_dict(), device.ordinal),
            *((CompileJob.create("b12x_next._lib.dense_gemm:_compile_dense_fused_quant_lowering",
                                 fused_b(config, device).to_dict(), device.ordinal),)
              if use_fused_b else ()),
            CompileJob.create(
                "b12x_next.gemm._shared.wo_mxfp8:compile_wo_quantizers",
