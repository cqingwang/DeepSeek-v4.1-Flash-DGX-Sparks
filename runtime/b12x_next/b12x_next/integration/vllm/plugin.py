"""Installable vLLM plugin that registers the b12x_next MX-FP6 (W6A6/W6A8) quantization.

Wire through vLLM's ``general_plugins`` entry point (see README)::

    [project.entry-points."vllm.general_plugins"]
    b12x_fp6 = "b12x_next.integration.vllm.plugin:register_b12x_fp6"
...
2. :class:`B12XFp6Config.override_quantization_method` claims a
   ``modelopt`` + ``quant_algo=W6A6`` checkpoint when ``B12X_NEXT_ENABLE_FP6=1``,
   so the launch script does not strictly need ``--quantization b12x_fp6``.
3. The checkpoint directory is resolved from ``B12X_NEXT_FP6_MODEL_DIR`` or from
   ``maybe_update_config(model_name=...)``.
4. ``get_quant_method`` dispatches each layer to the kernels in
   :mod:`b12x_next.integration.vllm.fp6_serving`:
     * ``FusedMoE``  -> ``_VllmMoEMethod`` -> :class:`B12XFP6MoEMethod`
       -> ``b12x_next.moe.fused_moe`` (``w6a8_mx``)
     * ``LinearBase``-> ``_VllmLinearMethod`` -> ``b12x_next::fp6_dense_linear``
...
from b12x_next.integration.vllm.fp6_serving import (
    QUANT_ALGO,
    QUANT_METHOD,
    B12XFP6MoEMethod,
...
MODEL_DIR_ENV = "B12X_NEXT_FP6_MODEL_DIR"
QUANT_NAME = "b12x_fp6"

# Unquantized bf16 linears with N <= MAX_OUT and K >= MIN_IN route through the
# b12x_next small-N GEMV.  See b12x_next.gemm.bf16_gemv for the thresholds.
from b12x_next.gemm.bf16_gemv import (  # noqa: E402
    SMALL_M_MAX,
...
def register_b12x_fp6() -> None:
    ...
    from vllm.utils.b12x_next import (
    ...
    def _b12x_fp6_linear(x, out_features, layer_name):
        ...
        return torch.ops.b12x_next.fp6_dense_linear(
    ...
    def _b12x_small_n_bf16_linear(x, out_features, layer_name):
        from b12x_next.gemm.bf16_gemv import mm
    ...
    class _FP6DenseProvider:
        ...
        def get_b12x_preparation_units(self, layer: Any, workload: Any):
            ...
            if workload.stage != "weights":
                return ()
            from b12x_next.preparation import PreparedCall
            from b12x_next.quantization.mxfp6._preparation import Mxfp6DenseQuery, plan as plan_query
            from b12x_next.quantization.mxfp6.fp6_dense_weights import _DENSE_PER_ROW_GS
    ...
    class _BF16GemvProvider:
        ...
        def get_b12x_preparation_units(self, layer: Any, workload: Any):
            ...
            if workload.stage != "weights":
                return ()
            from b12x_next.gemm.bf16_gemv import GemvQuery, plan as plan_query
            from b12x_next.preparation import PreparedCall
    ...
    class _VllmLinearMethod(LinearMethodBase):  # type: ignore[misc]
        ...
        def process_weights_after_loading(self, layer: Any) -> None:
            from b12x_next.quantization.mxfp6.fp6_checkpoint import (
                resolve_activation_format,
            )
            from b12x_next.quantization.mxfp6.fp6_dense_weights import PACKED_GEMM_MIN_N
            ...
            import b12x_next.quantization.mxfp6.fp6_dense_op  # noqa: F401
    ...
    class _VllmMoEMethod(FusedMoEMethodBase):  # type: ignore[misc]
        ...
        def process_weights_after_loading(self, layer: Any) -> None:
            from b12x_next.moe import fused_moe
    ...
    class B12XFp6Config(QuantizationConfig):  # type: ignore[misc]
        ...
        def _ensure_loaded(self) -> None:
            ...
            if not model_dir:
                raise RuntimeError(
                    ...
                    "B12X_NEXT_FP6_MODEL_DIR to the FP6 checkpoint path."
                )
            from b12x_next.quantization.mxfp6.fp6_checkpoint import WEIGHT_SCALE_SUFFIX
            from b12x_next.quantization.mxfp6.fp6_safetensors_load import (
                _source_format_from_config,
            )
            from b12x_next.quantization.mxfp6.model_fp6 import SafetensorsModel
            from b12x_next.quantization.mxfp6.fp6_checkpoint import (
        ...
        def _maybe_warn_zero_overlap(self, prefix: str) -> None:
            ...
            logger.warning(
                ...
                "(%d modules from %s). B12X_NEXT_FP6_MODEL_DIR likely points at a "
...
def _dense_weight_from_loaded(
    ...
    *,
    ...
    from b12x_next._lib.intrinsics import swizzle_block_scale
    from b12x_next.quantization.mxfp6.fp6_checkpoint import (
        activation_format_for_source,
        weight_format_for_source,
    )
    from b12x_next.quantization.mxfp6.fp6_dense_weights import FP6DenseWeight
