"""ModelOpt-mirror HF safetensors schema for b12x_next MX-FP6 (W6A6) checkpoints.

This module defines the *on-disk* contract for a b12x_next FP6 checkpoint and
...
(:mod:`b12x_next.quantization.mxfp6.model_fp6`) and any serving adapter
...
and ``quant_algo="W6A6"`` so a framework can route FP6 weights to the b12x_next
kernel.
"""
...
def build_quantization_config(
    *,
    ...
    can detect the scheme, while carrying the extra MX-FP6 specifics b12x_next
    needs.
    ...
    cfg: dict = {
        ...
        "producer": {"name": "b12x_next", "schema": SCHEMA_VERSION},
    }
...
def load_act_fmt_overrides(
    ...
    * ``B12X_NEXT_FP6_ACT_FMT_OVERRIDES`` — ``pat=fmt,pat=fmt`` (serve-time, no
      config edit).
    * ``quantization_config.activation_format_overrides`` — dict in
      ``config.json``.
    """
    env_spec = os.environ.get("B12X_NEXT_FP6_ACT_FMT_OVERRIDES", "").strip()
...
def _pack_codes(mat_bf16: torch.Tensor, fmt: str, *, use_gpu: bool) -> torch.Tensor:
    ...
    from b12x_next._lib.fp6 import quantize_grouped_mxfp6_torch
...
def _fp6_round_tables(
    ...
    from b12x_next._lib.fp6 import _decode_fp6_e2m3, _decode_fp6_e3m2
...
def _pack_codes_and_scales_mse(
    ...
    """
    from b12x_next._lib.fp6 import pack_fp6_codes_tensor
...
def dequantize_linear_from_fp6(
    ...
    *,
    ...
    """
    from b12x_next._lib.fp6 import expand_mxfp6_packed_to_bytes
