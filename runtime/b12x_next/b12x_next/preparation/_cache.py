...
def cache_identity(namespace: Mapping[str, object], device_ordinal: int):
    ...
    raw_version = os.environ.get("B12X_NEXT_TUNING_CACHE_VERSION", "1")
    try:
        version = int(raw_version)
    except ValueError:
        raise ValueError("B12X_NEXT_TUNING_CACHE_VERSION must be a positive integer") from None
    if version <= 0:
        raise ValueError("B12X_NEXT_TUNING_CACHE_VERSION must be a positive integer")
