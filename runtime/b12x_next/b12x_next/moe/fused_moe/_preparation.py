...
import torch
from b12x_next._lib.compile_plan import attach_programs, load_programs
from b12x_next._lib.compile_pool import CompileJob
from b12x_next.preparation.types import FrozenMapping, MemoryRequirements, Plan, _CompositePlan
...
def _w4a16_primary_launches(scratch, caps) -> _W4A16PrimaryLaunches:
    ...
    GPU invocation.
    """
    from b12x_next.moe._shared.kernels.w4a16.host import route_pack_capacity
...
def _compact_launches(plan, caps):
    if not (
        ...
        return None
    from b12x_next._lib.quant.mxfp8_rows import _get_compiled_mxfp8_rows_quant, mxfp8_rows_quant_launch_options
    from b12x_next._lib.utils import get_num_sm
    from b12x_next.moe._shared.kernels.w4a8_compact_micro import _compiled_direct_w4a8_compact
    from b12x_next.moe._shared.kernels.w4a16.kernel import compile_w4a16_topk_sum
...
def plan(experts: PreparedExperts, *, capacity: ExecutionCapacity, routing: RoutingSpec | None = None,
    ...
    def child(tokens):
        ...
        def compile_jobs(config, device):
            ...
            return (
                CompileJob.create(
                    "b12x_next.moe.fused_moe._preparation:compile_fused_moe",
                    TUNING.encode_query(query), TUNING.encode_config(config),
                    weight_payload, scale_counts, device.ordinal,
                ),
                CompileJob.create(
                    "b12x_next.moe.fused_moe._preparation:compile_route_topk",
                    ROUTE_TUNING.encode_query(route_query), device.ordinal,
                ),
                *(
                    CompileJob.create(
                        "b12x_next.moe.fused_moe._preparation:compile_dynamic_route_plan",
...
def compile_route_topk(query_payload, ordinal):
    ...
    import triton
    from b12x_next.moe._shared.routing import _route_topk_kernel
    from ._tuning import MoeRouteQuery
...
class _RouteTopKState:
    ...
    def run(self, router_logits, topk_logits, topk_ids, topk_weights, **kwargs):
        from b12x_next.moe._shared.routing import route_topk
        if (
...
def compile_fc2(query_payload, ordinal):
    ...
    from b12x_next.moe._shared.kernels.w4a16.kernel import _compile_w4a16_fc2_direct
...
def plan_fc2(experts: PreparedExperts, invocation: FC2Invocation, *, override=None,
    ...
    def jobs(config, device):
        return (CompileJob.create(
            "b12x_next.moe.fused_moe._preparation:compile_fc2",
