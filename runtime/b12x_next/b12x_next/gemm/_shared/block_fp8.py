...
def _check_block_size(block_size: Sequence[int]) -> tuple[int, int]:
    ...
    if (block_n, block_k) not in ((128, 128), (32, 32)):
        raise ValueError(
            f"b12x_next block FP8 linear supports 128x128 or 32x32 weight blocks, got {block_size}"
        )
...
def _c_dtype_name(dtype: torch.dtype) -> str:
    ...
    if dtype == torch.float16:
        return "float16"
    raise ValueError(
        f"b12x_next block FP8 linear output dtype must be bf16/fp16, got {dtype}"
    )
...
def pack_block_fp8_linear_weight_mxfp8(
    ...
    *,
    ...
    """Pack serialized block-FP8 linear weights for the native b12x_next MXFP8 GEMM.
