...
from b12x_next.preparation import DeviceIdentity, FrozenMapping
from b12x_next.preparation.tuning import Knob, ParameterBinding, ParameterSpace, TuningContract
...
def _default_config(query, device):
    ...
    threshold = int(query.controls.get("B12X_NEXT_MHC_PREFILL_MIN_TOKENS", "96"))
    ...
    if query.operation == "pre":
        ...
        splits = query.controls.get("B12X_NEXT_MHC_DECODE_SPLITS")
    ...
    partials = 4
    if prepared:
        raw = query.controls.get("B12X_NEXT_MHC_PARTIALS_PER_CTA")
        if raw not in (None, ""):
            partials = int(raw)
...
def _tf32_pin(query):
    value = query.controls.get("B12X_NEXT_MHC_PREFILL_TF32_MMA")
    if value is None:
        value = query.controls.get("B12X_NEXT_MHC_PREFILL_BF16_MMA")
...
def _validate_query(query, device):
    ...
    if query.controls.get("B12X_NEXT_AUTOTUNE_EXHAUSTIVE", "0") not in ("0", "1"):
        raise ValueError("B12X_NEXT_AUTOTUNE_EXHAUSTIVE must be 0 or 1")
...
def _tuning_parameters(query: MhcQuery, device: DeviceIdentity | None):
    ...
    partials = query.controls.get("B12X_NEXT_MHC_PARTIALS_PER_CTA")
    values = {
        "backend": backends,
    ...
    return ParameterSpace.create(
        TUNING.knobs,
        values=values,
        exhaustive=query.controls.get("B12X_NEXT_AUTOTUNE_EXHAUSTIVE", "0") == "1",
        predicates=(
