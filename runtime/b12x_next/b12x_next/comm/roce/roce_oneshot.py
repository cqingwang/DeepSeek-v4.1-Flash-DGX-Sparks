...
def default_gid_index() -> int:
    """``B12X_NEXT_ROCE_GID_INDEX``, else NCCL's ``NCCL_IB_GID_INDEX``, else 3."""

    return _env_int(
        "B12X_NEXT_ROCE_GID_INDEX", "NCCL_IB_GID_INDEX", default=DEFAULT_GID_INDEX
    )
...
def discover_hcas(gid_index: Optional[int] = None) -> tuple[str, ...]:
    ...
    ``B12X_NEXT_ROCE_HCA`` (or NCCL's ``NCCL_IB_HCA``) selects explicitly; otherwise
    every active device with a populated GID at ``gid_index`` is used.
    """

    explicit = _env_list("B12X_NEXT_ROCE_HCA", "NCCL_IB_HCA")
    if explicit:
        return explicit[:2]
...
class RoceOneshotAllReduce:
    ...
    def __init__(
        ...
        *,
        ...
        self.spin_limit = _env_int("B12X_NEXT_ROCE_SPIN_LIMIT", default=DEFAULT_SPIN_LIMIT)
