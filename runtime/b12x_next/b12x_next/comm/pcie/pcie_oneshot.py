...
def _push_mode_enabled() -> bool:
    """Match the extension's B12X_NEXT_PCIE_ONESHOT_PUSH transport toggle.
...
def _tp2_plain_remote_push_enabled() -> bool:
    ...
    configured = os.getenv("B12X_NEXT_PCIE_TP2_PLAIN_REMOTE_PUSH")
    if configured is None:
        configured = os.getenv("B12X_NEXT_PCIE_TP2_REMOTE_PUSH", "1")
...
def _tp4_remote_push_enabled() -> bool:
    ...
    return os.getenv("B12X_NEXT_PCIE_TP4_REMOTE_PUSH", "0") not in ("", "0")


def _tp8_owner_reduce_enabled() -> bool:
    """Enable the qualified topology-scoped fused TP8 transport by default."""

    return os.getenv("B12X_NEXT_PCIE_TP8_OWNER_REDUCE", "1") not in ("", "0")
...
def _require_full_grid_residency(
    *,
    ...
    try:
        ...
        test_visible_sms = int(os.getenv("B12X_NEXT_PCIE_TEST_VISIBLE_SM_COUNT", "0") or "0")
...
class _CuTeOneshotBackend:
    ...
    def _launch_geometry(size_packs: int) -> tuple[int, int]:
        threads = int(os.getenv("B12X_NEXT_PCIE_ONESHOT_THREADS", "256"))
        threads = min(512, max(64, (threads // 32) * 32))
        block_limit = int(os.getenv("B12X_NEXT_PCIE_ONESHOT_BLOCK_LIMIT", "8"))
        if block_limit <= 0 or block_limit > _MAX_BLOCKS:
            raise ValueError(
                f"B12X_NEXT_PCIE_ONESHOT_BLOCK_LIMIT must be in [1, {_MAX_BLOCKS}]"
            )
    ...
    def _fused_threads() -> int:
        threads = int(os.getenv("B12X_NEXT_PCIE_FUSED_THREADS", "256"))
    ...
    def _fused_launch_config(
        ...
        override = int(os.getenv("B12X_NEXT_PCIE_FUSED_CTAS_PER_ROW", "0"))
        if override > 0:
