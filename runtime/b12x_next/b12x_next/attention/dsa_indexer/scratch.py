...
from b12x_next.attention.dsa_indexer.msa_reference import (
    MSA_BLOCK_TOKENS,
)
...
_INDEXER_STREAM_SCORER_ENV = "B12X_NEXT_INDEXER_STREAM_SCORER"
    ...
    *,
    ...
    if route == INDEXER_PAGED_ROUTE_AUTO:
        if bool(caps.shared_page_table) or str(caps.mode) == "prefill":
            ...
            if os.getenv("B12X_NEXT_FUSED_INDEXER", "1") != "0":
    ...
    if route == INDEXER_PAGED_ROUTE_PACKED_CONTIGUOUS:
        ...
        width = int(caps.max_page_table_width) * int(caps.page_size)
        if not (
            os.getenv("B12X_NEXT_FUSED_INDEXER", "1") != "0"
