...
static void delete_batch(PyObject *capsule) {
    batch_executor_t *executor = PyCapsule_GetPointer(capsule, "b12x_next.batch_executor");
    if (!executor) return;
...
static PyObject *py_batch_executor(PyObject *self, PyObject *args) {
    ...
    PyObject *capsule = PyCapsule_New(executor, "b12x_next.batch_executor", delete_batch);
    if (!capsule) {
        bounce_release(executor->bounce);
        pthread_mutex_destroy(&executor->mutex);
        free(executor);
