...
from b12x_next.preparation.types import FrozenMapping, MemoryRequirements, Plan, require_prepared
...
def compile_cache_writer(payload, ordinal):
    ...
    from torch._subclasses.fake_tensor import FakeTensorMode
    from b12x_next._lib.compile_plan import compile_only_launches
    from b12x_next.attention._shared.mla import kv_cache as writer
...
def _query_from_caps(caps: Caps, invocation: FrozenMapping) -> SparseMlaQuery:
    ...
    raw_prefill_mg_enabled = invocation.get(
        "prefill_mg_enabled",
        os.environ.get("B12X_NEXT_MLA_SM120_PREFILL_MG", "1")
...
class _SparseMlaState:
    ...
    def run(self, binding: B12XSparseMLABinding, *, kv_cache, attention_sink=None):
        ...
        if self.caps.mode == "decode":
            from b12x_next.attention._shared.mla.kernel import run_prepared_unified_decode
    ...
    *,
    ...
    return Plan(
        ...
        _compile_jobs=lambda config, device: (CompileJob.create(
            "b12x_next.attention.sparse_mla._preparation:compile_sparse_mla",
...
def plan_cache_writer(
    ...
    *,
    ...
    return Plan(
        contract=TUNING, query=query, invocation=invocation, override=override,
        _compile_jobs=lambda config, device: (CompileJob.create(
            "b12x_next.attention.sparse_mla._preparation:compile_cache_writer",
