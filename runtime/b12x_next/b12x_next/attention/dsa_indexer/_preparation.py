...
from b12x_next._lib.compile_plan import attach_programs, compile_only_launches, load_programs
...
def plan(caps, *, invocation: FrozenMapping = FrozenMapping(), override: DsaIndexerConfig | None = None) -> Plan:
    ...
    def compile_jobs(config, device):
        return (CompileJob.create("b12x_next.attention.dsa_indexer._preparation:compile_indexer", TUNING.encode_query(replace(query, exhaustive=False)), TUNING.encode_config(config), device.ordinal),)
