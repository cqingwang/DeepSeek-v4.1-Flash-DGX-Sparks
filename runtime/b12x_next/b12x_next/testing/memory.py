...
def absorb_small_page_fragments(device: int = 0) -> int:
    ...
    No-op on discrete GPUs. ``B12X_NEXT_BENCH_SPONGE=0`` disables it and a number
    sets a fixed size in GB; otherwise the size follows the current fragment
    inventory, always leaving at least 16 GB free.
    """
    request = os.environ.get("B12X_NEXT_BENCH_SPONGE", "auto").strip().lower()
    ...
    _SPONGE.append(ptr)
    print(
        f"[b12x_next] holding {size / (1 << 30):.1f} GB of small-page device memory "
        f"(B12X_NEXT_BENCH_SPONGE={request}) so benchmark buffers land on 2 MB pages",
        file=sys.stderr,
    )
    return size
