...
from b12x_next._lib.utils import cuda_stream_to_int, get_num_sm
from b12x_next.gemm.wo_projection._tuning import WoProjectionConfig
...
_WO_QUANT_CHUNKS_PER_PROGRAM = int(
    os.environ.get("B12X_NEXT_WO_QUANT_CHUNKS_PER_PROGRAM", "16")
)
if _WO_QUANT_CHUNKS_PER_PROGRAM not in (1, 2, 4, 8, 16, 32):
    raise ValueError(
        "B12X_NEXT_WO_QUANT_CHUNKS_PER_PROGRAM must be one of 1, 2, 4, 8, 16, or 32, got "
...
class WOProjectionBinding:
    ...
    def run(self, *, stream: object = None) -> torch.Tensor:
        from b12x_next.preparation.types import require_prepared
...
def pack_mxfp8_scales_for_dense_gemm(
    ...
    *,
    ...
    """Pack compact MXFP8 row/chunk scales into b12x_next dense-GEMM MMA layout.
...
def _requantize_block_fp8_to_ue8m0(
    ...
    *,
    ...
    block_amax / 448` is an *arbitrary* fp32 128x128 block scale. The b12x_next MMA
...
@torch.library.custom_op(
    "b12x_next::quantize_wo_a_input_inv_rope_mxfp8_alloc",
    mutates_args=(),
)
...
@torch.library.custom_op(
    "b12x_next::quantize_wo_a_input_inv_rope_mxfp8_launch",
    ...
    *,
    ...
    if out is None:
        values_base, scale_rows_base, scale_physical_base = (
            torch.ops.b12x_next.quantize_wo_a_input_inv_rope_mxfp8_alloc(
                o,
                positions,
                cos_sin_cache,
                tokens,
    ...
    _check_mxfp8_rows_storage(out, m=tokens, k=group_width, num_groups=groups)
    torch.ops.b12x_next.quantize_wo_a_input_inv_rope_mxfp8_launch(
        o,
        positions,
        cos_sin_cache,
        out.values,
...
@torch.library.custom_op(
    "b12x_next::quantize_wo_b_input_mxfp8_alloc",
    mutates_args=(),
)
...
@torch.library.custom_op(
    "b12x_next::quantize_wo_b_input_mxfp8_launch",
    mutates_args=("out_values", "out_scale_rows", "out_scale_mma"),
)
...
def quantize_wo_b_input_mxfp8(
    ...
    *,
    ...
    if out is None:
        values_base, scale_rows_base, scale_physical_base = (
            torch.ops.b12x_next.quantize_wo_b_input_mxfp8_alloc(
                source_trg,
                tokens,
                rank,
                groups,
    ...
    torch.ops.b12x_next.quantize_wo_b_input_mxfp8_launch(
        source_trg,
        out.values,
        out.scale_rows,
        out.scale_mma,
...
def _wo_b_fused_tiled_plan(
    ...
    override = os.getenv("B12X_NEXT_WO_B_FUSED_TILE", "").strip().lower()
...
@torch.library.custom_op(
    "b12x_next::wo_projection_inv_rope_mxfp8_fused",
    mutates_args=(),
)
...
def wo_projection_inv_rope_mxfp8(
    ...
    *,
    ...
    output = torch.ops.b12x_next.wo_projection_inv_rope_mxfp8_fused(
        o,
        positions,
        cos_sin_cache,
        weights.wo_a.values,
