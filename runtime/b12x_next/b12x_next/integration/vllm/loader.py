...
from b12x_next.loader._checkpoint import DirectWeightSession
from b12x_next.loader._progress import CheckpointDisplay

logger = init_logger("vllm.model_executor.model_loader.b12x_next")
...
class B12xModelLoader(DefaultModelLoader):
    ...
    def __init__(self, load_config):
        ...
        if options.get("enable_multithread_load"):
            raise ValueError("b12x_next currently uses synchronous checkpoint routing")
        if load_config.safetensors_load_strategy not in (None, "lazy"):
            raise ValueError(
                "b12x_next O_DIRECT input does not use safetensors read strategies"
            )
        super().__init__(
            dataclasses.replace(
                load_config,
    ...
    def load_model(self, vllm_config, model_config, prefix=""):
        ...
        if device.type != "cuda":
            raise ValueError("the b12x_next loader requires a CUDA device")
        index = torch.cuda.current_device() if device.index is None else device.index
        shared_read_group = None
        read_mode = self.read_mode
        if read_mode == "auto":
            from b12x_next.loader import capabilities

            read_mode = "bounce" if capabilities(index)["host_page_tables"] else "gds"
        if read_mode == "gds":
            from vllm.distributed.parallel_state import get_tp_group
            from b12x_next.loader._shared_checkpoint import SharedReadGroup
        ...
        logger.debug("b12x_next O_DIRECT I/O counters: %s", io_stats)
        load_seconds = (
            self.counter_after_loading_weights - self.counter_before_loading_weights
        )
        payload_gb = io_stats["payload_bytes"] / 1e9
        logger.debug(
            "b12x_next effective weight loading: %.2f GB in %.2f s = %.2f GB/s "
    ...
    def _get_weights_iterator(self, source):
        ...
        if self._session is None:
            raise RuntimeError("b12x_next requires an active initial-load session")
...
def register_b12x_loader():
    ...
    register_model_loader("b12x_next")(B12xModelLoader)
