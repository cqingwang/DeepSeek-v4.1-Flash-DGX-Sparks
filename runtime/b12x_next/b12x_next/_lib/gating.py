"""Capability gating for b12x_next.
...
def require_cutlass_dsl(minimum: str = MIN_CUTLASS_DSL) -> None:
    ...
    if version is None:
        raise ImportError(
            "b12x_next requires nvidia-cutlass-dsl "
    ...
    if _version_tuple(version) < _version_tuple(minimum):
        raise ImportError(
            f"b12x_next requires nvidia-cutlass-dsl >= {minimum}, "
...
def require_triton(op: str) -> None:
    if not has_triton():
        raise ImportError(
            f"b12x_next.{op} requires triton (bundled with "
